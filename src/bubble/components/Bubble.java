...
public class Bubble extends JLabel implements Moveable {
	...
	public void left() {
		...
		for(int i = 0; i < 400; i++) {
			...
			System.out.println("적군 x 좌표 위치 : " + mContext.getEnemy1().getX());
			System.out.println("적군 y 좌표 위치 : " + mContext.getEnemy1().getY());
			// x 절대값만 확인해 보자. 
			...
			int absX2 =  Math.abs(x - mContext.getEnemy2().getX());
			int absY2 =  Math.abs(y - mContext.getEnemy2().getY());
			if( absX2 < 10 && absY2 < 50 ) {
				// 단. 적군이 살아 있을 때 crash() 메서드 호출이 되어야 함.
				if(mContext.getEnemy2().getState() == 0) {
					crash2();
				}
			}
			int absX3 =  Math.abs(x - mContext.getEnemy3().getX());
			int absY3 =  Math.abs(y - mContext.getEnemy3().getY());
			if( absX3 < 10 && absY3 < 50 ) {
				// 단. 적군이 살아 있을 때 crash() 메서드 호출이 되어야 함.
				if(mContext.getEnemy3().getState() == 0) {
					crash3();
				}
			}
	...
	public void crash2() {
		mContext.getEnemy2().setState(1);
		state = 1;
		setIcon(bubbled);
		mContext.remove(mContext.getEnemy2());
		
		// 버블에 이미지를 변경 처리 
	}
	public void crash3() {
		mContext.getEnemy3().setState(1);
		state = 1;
		setIcon(bubbled);
		mContext.remove(mContext.getEnemy3());
